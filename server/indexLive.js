...
app.use(session({ key: 'username', secret: 'ambalabanijjopluscom', resave: false, saveUninitialized: false, store: this.store}));
const dbConnection = mysql.createConnection ({
 host: 'localhost',
 user: 'microfin_ecommerce',
 password: 'sikder!@#',
...
app.get('/api/delete-vat-tax', verifyToken, async function(req, res, next) {
    ...
    jwt.verify(req.token, 'secretkey', async function (err, authData) {
      if (err) {
        ...
        return res.send({ success: true, message: 'category list.' });
...
app.get('/api/get-product-edit', verifyToken, async function (req, res) {
  ...
  jwt.verify(req.token, 'secretkey', async function (err, authData) {
    if (err) {
      ...
      var all_specification_name = await query('SELECT * FROM product_specification_names WHERE status = 1');
      ...
      data = {
        ...
        'description'     : selected_products[0].product_full_description,
        'images' : selected_products[0].image
      };
  ...
  jwt.verify(req.token, 'secretkey', (err, authData) => {
    if (err) {
      ...
      try {
        ...
        // console.log('Meta Tags are : ', req.body.metaTags);
        // for (var i = 0; i < req.body.metaTags.length; i++) {
        //   var tags = JSON.parse(req.body.metaTags[i]);
        //   metaTags.push(tags.displayValue);
        //
        //   console.log('Meta Tags : ', tags.displayValue);
        // }
        ...
        var insert_sql_query = "INSERT INTO products (product_name, category_id, product_sku, productPrice, brand_name, product_specification_name, product_specification_details_description, product_full_description, qc_status, image,home_image, vendor_id, status, isApprove) VALUES ('"+req.body.productName+"', '"+req.body.categoryIdValue+"', '"+req.body.productSKUcode+"', '"+req.body.productPrice+"', '"+req.body.productBrand+"', '"+JSON.stringify(specifiationOBJ)+"','"+JSON.stringify(specifiationOBJ)+"' , '"+req.body.productDescriptionFull+"', '1', '"+req.body.productImagesJson+"','"+req.body.homeImage+"', '"+req.body.vendor_id+"', '1', 2 )";
      ...
      try {
        ...
        if (req.body.homeImage !== 'NAN') {
          ...
          setTimeout(()=> {
            ...
            var update_sql_query_for_product = "UPDATE products SET product_name = '"+req.body.productName+"', productPrice = '"+req.body.productPrice+"', brand_name = '"+req.body.productBrand+"', image = '"+JSON.stringify(dataForImages)+"', home_image = '"+req.body.homeImage+"', isApprove = 2 WHERE id = '"+req.body.getEditId+"'";
        ...
        if (JSON.parse(req.body.productImagesJson).length > 0) {
          ...
          setTimeout(()=> {
            if (checkCounter == 0) {
              ...
              var update_sql_query_for_product = "UPDATE products SET product_name = '"+req.body.productName+"', productPrice = '"+req.body.productPrice+"', brand_name = '"+req.body.productBrand+"', image = '"+JSON.stringify(dataForImages)+"', isApprove = 2 WHERE id = '"+req.body.getEditId+"'";
        ...
        if (req.body.homeImage === 'NAN' && JSON.parse(req.body.productImagesJson).length == 0) {
          var update_sql_query_for_product = "UPDATE products SET product_name = '"+req.body.productName+"', productPrice = '"+req.body.productPrice+"', brand_name = '"+req.body.productBrand+"', isApprove = 2 WHERE id = '"+req.body.getEditId+"'";
...
app.post('/api/saveProductPurchase', verifyToken, (req, res) => {
  ...
  jwt.verify(req.token, 'secretkey', (err, authData) => {
    if (err) {
      ...
      promise = new Promise (function (resolve, reject) {
        ...
        try {
          var insert_sql_query = "INSERT INTO inv_purchase (billNo, chalanNo, supplierId, storedBy, purchaseDate, totalQuantity, totalAmount, status) VALUES ('"+req.body.currentBillNo+"', '"+req.body.chalanNo+"', '"+req.body.vendorId+"', '"+req.body.storedBy+"', '"+req.body.currentDate+"', '"+req.body.grandTotalQuantity+"', '"+req.body.grandTotalPrice+"', '1')";

          dbConnection.query(insert_sql_query, function (err, result) {
            console.log('user insert result : ', result.insertId);
            console.log('user error result : ', err);
            if (result) {
              console.log("1 record inserted to Purchase Table");
              // return res.send({success: true, server_message: result});
        ...
        async.forEachOf(purchaseElements, function (purchaseElement, i, inner_callback){
          ...
          dbConnection.query(insert_sql_query, function(err, results, fields){
            if(!err){
              console.log("Query Results For Purchase Details : ", results);
              inner_callback(null);
            } else {
...
app.get('/api/deleteColor', async function (req, res) {
  try {
...
app.get('/api/deleteSizeType', async function (req, res) {
  try {
...
app.get('/api/vendorAuthorization', async function (req, res)  {

  console.log('Vendor id for authorization : ', req.query.id);

  console.log('Vendor id for authorization : ', req.query.clicked);
