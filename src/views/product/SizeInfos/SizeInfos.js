...
class SizeInfos extends Component {
  constructor(props) {
    super(props);
  ...
  toggleSmall(event) {
    if (event == 'Yes') {
      ...
      fetch(base+`/api/deleteSizeInfo/?id=${this.state.colorId}`, {
        method: 'GET',
  ...
  editItem (event) {
    ...
    setTimeout(() => {
      fetch(base+`/api/getSizeInfoForUpdate/?id=${this.state.colorId}`, {
        method: 'GET',
        headers: {'Authorization': 'Atiq '+cookie.load('token')}
      })
      .then(res => {
      ...
      .then(infos => {
        ...
        if (infos.success == true) {
          this.setState({
            size: infos.data['size'],
            sizeType: infos.data['size_type_id']
          })
        }
        else {
          ToastsStore.warning(infos.message);
  ...
  render() {
    ...
    return (
      ...
      <Col xs="12" md="6">
        <Card>
          <CardHeader>
            <strong> {this.state.buttonName == 'submitButton'? 'Add New Size' : 'Uppdate Size'} </strong>
          </CardHeader>
          ...
          <CardBody>
            <Form action="" method="post" encType="multipart/form-data" onSubmit={this.handleSubmit} onChange={this.handleChange} className="form-horizontal">
              ...
              <FormGroup row>
                <Col md="3">
                  <Label htmlFor="vendorList">Size Type</Label>
                </Col>
                <Col xs="12" md="9">
                  <Input type="select" name="sizeType" id="sizeType" onChange={this.handleChange.bind(this)} value={this.state.sizeType}>
              ...
              <FormGroup row>
                <Col md="3">
                  <Label htmlFor="categoryName">Size</Label>
                </Col>
                <Col xs="12" md="9">
                  <Input type="text" id="size" name="size" value={this.state.size} required="true" placeholder="Size" />
                </Col>
              </FormGroup>
      ...
      <Col xs="12" lg="6">
            <Card>
              <CardHeader>
                <Row>
                  <Col md="12">
                    <i className="fa fa-align-justify"></i> Size Info List
                  </Col>
                </Row>
              </CardHeader>
                  ...
                  <tbody>
                    {
                      ...
                      <tr>
                        <td>{sizeListValue.size}</td>
                        <td>{sizeListValue.name}</td>
                        <td>
                          <center>
                            <a href="#" onClick={this.editItem.bind(this)} id="deleteIds" ref="dataIds" data-id={sizeListValue.id}>
                              <i className="fa fa-edit fa-lg"  title="Edit Color" aria-hidden="true" style={{color: '#009345'}}></i>
                            </a>
                            {' '}
                            <a href="#" onClick={this.deleteItem.bind(this)} id="deleteIds" ref="dataIds" data-id={sizeListValue.id}>
          ...
          <Modal isOpen={this.state.small} toggle={this.toggleSmall}
            ...
            <ToastsContainer store={ToastsStore}/>
            <ModalHeader toggle={this.toggleSmall}>Delete Size Infos</ModalHeader>
            <ModalBody>
              Are You Sure To Delete This Size Infos ?
            </ModalBody>
            <ModalFooter>
