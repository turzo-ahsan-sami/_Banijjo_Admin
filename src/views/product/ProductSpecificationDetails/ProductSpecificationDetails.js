...
class ProductSpecificationDetails extends Component {
  ...
  handleGenarateDataTable () {
    fetch(base+`/api/product_specification_details/?type=${this.state.userType}&id=${this.state.userId}`, {
      method: 'GET'
    })
    .then(res => {
      console.log(res);
      ...
      .then((info) => {
        if (info.success == true) {
          ...
          if (info.status == 403) {
            ...
            ToastsStore.warning("Product Specificaton Details Insertion Faild. Please try again !!");
            console.log(info.success);
          }
