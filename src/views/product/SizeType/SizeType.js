...
class SizeType extends Component {
  ...
  toggleSmall(event) {
    if (event == 'Yes') {
      ...
      fetch(base+`/api/deleteSizeType/?id=${this.state.colorId}`, {
        method: 'GET',
  ...
  handleSubmit(event) {
    ...
    if (this.state.buttonPermittedFor == 'submit') {
      ...
      fetch(base+'/api/editSizeTypeInfos' , {
        method: "POST",
        headers: {
          'Content-type': 'application/json',
          'Authorization': 'Atiq '+cookie.load('token')
  ...
  editItem (event) {
    ...
    setTimeout(() => {
      fetch(base+`/api/getSizeTypeInfoForUpdate/?id=${this.state.colorId}`, {
        method: 'GET',
        headers: {'Authorization': 'Atiq '+cookie.load('token')}
      })
      .then(res => {
  ...
  render() {
    ...
    return (
      ...
      <Col xs="12" md="6">
        <Card>
          <CardHeader>
            <strong>{this.state.buttonName == 'submitButton'? 'Add New Size Type' : 'Uppdate Size Type'}</strong>
          </CardHeader>
          ...
          <CardBody>
            <Form action="" method="post" encType="multipart/form-data" onSubmit={this.handleSubmit} onChange={this.handleChange} className="form-horizontal">
              ...
              <FormGroup row>
                <Col md="3">
                  <Label htmlFor="categoryName">Size Type Name</Label>
                </Col>
                <Col xs="12" md="9">
                  <Input type="text" id="name" name="name" value={this.state.name} required="true" placeholder="Size Type" />
                </Col>
              </FormGroup>
      ...
      <Col xs="12" lg="6">
            <Card>
              <CardHeader>
                <Row>
                  <Col md="12">
                    <i className="fa fa-align-justify"></i> Size Type List
                  </Col>
                </Row>
              </CardHeader>
          ...
          <Modal isOpen={this.state.small} toggle={this.toggleSmall}
            ...
            <ToastsContainer store={ToastsStore}/>
            <ModalHeader toggle={this.toggleSmall}>Delete Size Type</ModalHeader>
            <ModalBody>
              Are You Sure To Delete This Size Type ?
            </ModalBody>
            <ModalFooter>
