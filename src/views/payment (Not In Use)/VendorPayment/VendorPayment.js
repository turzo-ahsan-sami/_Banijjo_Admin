...
class VendorPayment extends Component {
  ...
  updateClicked (event) {
    ...
    .then(deliveryEdit => {
      ...
      for (let i = 0; i < deliveryEdit.data.length; i++) {
        ...
        this.state.products.push(<div><Input type="checkbox" name="products" style={{marginLeft: "0.10rem"}} value={deliveryEdit.data[i].product_id} checked/> <div style={{marginLeft: "20px"}}>{deliveryEdit.data[i].product_name}</div>  </div>);
  ...
  productCheckList(event) {
    ...
    for (var i = 0; i < event.length; i++) {
      ...
      this.state.products.push(<div><Input type="checkbox" name="products" style={{marginLeft: "0.10rem"}} value={event[i].product_id} /> <div style={{marginLeft: "20px"}}>{event[i].product_name}</div>  </div>);
    }
                  ...
                  <Input type="select" name="salesBillNo" id="salesBillNo" value={this.state.salesBillNo} onChange={this.handleSalesBillWiseProduct.bind(this)} >
                    <option value="0">Please select</option>
                    {
                      this.state.sales_bill_no.map((sales_bill_no_value, key) =>
                        <option value={sales_bill_no_value.id}> {sales_bill_no_value.bill_no} </option>
                      )
                    }
                  </Input>
                </Col>
