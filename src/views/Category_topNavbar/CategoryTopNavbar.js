...
class CategoryOrder extends Component {
  ...
  toggleSmall(event) {
    ...
    if (event == 'deleteCategoryOrderListPermitted') {
      fetch(base+'/api/nav_category_order_list_delete', {
        method: 'GET',
  ...
  getFilterCategory (event) {
    ...
    fetch(base+`/api/search_filter_nav_categories/`, {
      method: 'GET',
      headers: {'Authorization': 'Atiq '+cookie.load('token')}
    })
    .then(res => {
  ...
  searchCategory (event) {
    ...
    if ( event.target.value != '') {
      ...
      fetch(base+`/api/search_nav_category_for_order_list/?search_string=${event.target.value}`, {
        method: 'GET',
        headers: {'Authorization': 'Atiq '+cookie.load('token')}
      })
      .then(res => {
  ...
  addClick(){
    ...
    if (this.state.PurchaseList.length < 8) {
  ...
  limitExitMessage () {
    this.state.limitExitMessageString = 'Limitation crossed ! Maximum size is 8 !';
  }
  ...
  getCategoryOrderList () {
    fetch(base+'/api/nav_category_order_list', {
      method: 'GET'
    })
    .then(res => {
      console.log(res);
  ...
  handleSubmit(event) {
    ...
    if (this.state.PurchaseList.length == 8) {
      if (this.state.effectiveDate) {
        fetch(base+'/api/save_selected_nav_category_order' , {
          method: "POST",
          headers: {
      ...
      ToastsStore.warning('Limitation Issue !! The size is : '+this.state.PurchaseList.length+' Need At Least 8');
  ...
  render() {
    ...
    return (
      ...
      <Col xs="12" md="6">
        <Card>
          <CardHeader>
            <strong>Add New Top Navbar Category Order</strong>
          </CardHeader>
      ...
      <Col xs="12" lg="6">
        <Card>
          ...
          <CardHeader>
            <Row>
              <Col md="6">
                <i className="fa fa-align-justify"></i>&nbsp; Top Navbar Category Order
              </Col>
              <Col md="6">
              </Col>
            </Row>
