  ...
  updateClicked () {
    ...
    .then(categoryOrderList => {
      ...
      for (let i = 0; i < categoryOrderList.data.length; i++) {
        ...
        this.setState({
          effectiveDate: categoryOrderList.data[i].effectiveDate
        })
  ...
  handleSubmit(event) {
    ...
    if (this.state.PurchaseList.length == 1) {
      if (this.state.effectiveDate) {
        console.log('Effective Date : ', this.state.effectiveDate);
                    ...
                    <td>
                      {
                        this.state.categoryOrderList.length > 0 ?
                          this.state.categoryOrderList.map((categoryOrderListValue, key) =>
                            <li><span>{categoryOrderListValue.category_name}</span> - <span style={{width: "35px", height: "20px", border: "1px solid green"}}>{categoryOrderListValue.effectiveDate}</span></li>
                          )
                        :
